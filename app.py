...
def create_view(
    ...
    over10_rsPCP = " AND rsPCP_Adjusted > 10"
    over20_rsPCP = " AND rsPCP_Adjusted > 20"
    over10_psPCP = " AND psPCP > 10"
    over20_psPCP = " AND psPCP > 20"
...
def index():
    ...
    if request.method == "POST":
        selected_season = request.form.get("season", default_season)
        view_type = request.form.get("view_type", "default")
        player_search = request.form.get("player_search", "")
        ...
        if selected_season:
            result_df = get_data(
                selected_season, view_type=view_type, player_search=player_search
            )

    table_html = (
        result_df.to_html(
            classes="table table-striped", index=False, table_id="stats-table"
        )
